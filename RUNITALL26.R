...
parameters <- read.csv("input34.csv")
parameters <- parameters[901:1800,]

# subset - do in chunks
# parameters <- read.csv("input34.csv")[1:1000,]
...
clusterExport(cl, c("BLANK26", "GROW_SAV26", "GROW_FP26", 
                    "INPUT25","MOVE25","OUTPUT26_nutr",
                    "OVERWINTER25","SPECIES25","START26",
                    "STEP26","UPTAKE_N26","UPTAKE_P26","WIND25"))
...
RESULT <- foreach (i=1:nrow(parameters), .combine=rbind, .errorhandling='pass') %dopar% { # loop through all of your simulations - User needs to specify the max # of simulations (rows of parameters) in .csv
  ...
  OUTPUT26_nutr(animate=FALSE,regimethreshold=70) 
...
write.csv(parameters,"output34b.csv",row.names=F)  
