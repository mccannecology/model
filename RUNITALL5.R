...
parameters <- read.csv("input18a.csv") 
...
RESULT <- foreach (i=1:nrow(parameters), .combine=rbind) %dopar% { # loop through all of your simulations - User needs to specify the max # of simulations (rows of parameters) in .csv
  ...
  INPUT5(simulnumb) # reads the .csv file of parameter values and assigns them to the global environment 
...
write.csv(parameters,"output18a.csv",row.names=F) 
