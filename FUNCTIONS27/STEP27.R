...
STEP27<-function(x,t){
  ...
  if (t %ni% winters){
    ...
    # Grow SAV #
    ############
    nextstep$SAV <- GROW_SAV27(thisstep$SAV,
                               thisstep$FPtotal,
                               thisstep$TOTALP,
                               thisstep$TOTALN,
                               LAND) 
    ...
    for (j in 1:numbFPspecies) {
      nextstep$FP[[j]]<-GROW_FP27(thisstep$FP,
                                  thisstep$FPtotal,
                                  j,
                                  thisstep$TOTALP,
                                  thisstep$TOTALN,
    ...
    nextstep$SAV <- MOVE27(nextstep$SAV,neigh_thresh_SAV,focal_thresh_SAV,amnt_colonize_SAV)
    ...
    nextstep$FP <- lapply(nextstep$FP,function(x){
      MOVE27(x,neigh_thresh_FP,focal_thresh_FP,amnt_colonize_FP)
    })
    ...
    nextstep$FP <- lapply(nextstep$FP,function(x){
      WIND27(x,proptomove,direction_of_wind,full_thresh_wind=800) 
    })
  ...
  ##############
    ...
    for (i in 1:numbFPspecies) {
      nextstep$FP[[i]]<-OVERWINTER27(thisstep$FP,i) 
    }
