...
RELEASE_P <- function(x,i) {
  newbiomass <- sum(x[[i+1]]$SPALLmatrix-x[[i]]$SPALLmatrix) # new biomass that grew on most recent 
  
  x[[i+1]]$TOTALP <- x[[1]]$TOTALP # this just resets TOTALN to its initial value - need subtract out biomass on i+1 - biomass on 1
