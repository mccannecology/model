...
WIND21 <- function (x1,proptomove,direction_of_wind,full_thresh_wind=800) {
        ...
        if ((j>=2 & j<=(height-1)) & (k>=2 & k<=(width-1))) {                                           
          ...
          if (direction_of_wind == "U"){  
            if (x1[j-1,k] < full_thresh_wind) { # only move if the destination cell is not "too full" 
              x2[j-1,k] <- x1[j-1,k] + amounttomove # add amountomove UP
              x2[j,k] <- x1[j,k] - amounttomove # subtract this value from the original spot 
            }
          }
          if (direction_of_wind == "R"){    
            if (x1[j,k+1] < full_thresh_wind) { # only move if the destination cell is not "too full" 
            x2[j,k+1] <- x1[j,k+1] + amounttomove # add amountomove RIGHT
            x2[j,k] <- x1[j,k] - amounttomove # subtract this value from the original spot 
            }
          }
          if (direction_of_wind == "D"){
            if (x1[j+1,k] < full_thresh_wind) { # only move if the destination cell is not "too full" 
              x2[j+1,k] <- x1[j+1,k] + amounttomove # add amountomove DOWN
              x2[j,k] <- x1[j,k] - amounttomove # subtract this value from the original spot 
            }
          }
          if (direction_of_wind == "L"){
            if (x1[j,k-1] < full_thresh_wind) { # only move if the destination cell is not "too full" 
              x2[j,k-1] <- x1[j,k-1] + amounttomove # add amountomove LEFT
              x2[j,k] <- x1[j,k] - amounttomove # subtract this value from the original spot 
            }
          }
