STEP13 <- function() { # start defining the function 
  ...
  for (i in 1:((timesteps+1)*years)) { # loop through time steps 
    ...
    if (i %ni% winters) { 
      ...
      # FP growth
      if (numbFPspecies == 4) { 
        LIST[[i+1]]$SAVmatrix <- GROW_SAV3(LIST[[i]]$SAVmatrix, LIST[[i+1]]$SAVmatrix, LIST[[i]]$FPALLmatrix, LIST[[i]]$TOTALP, LIST[[i]]$TOTALN) 
        LIST[[i+1]]$FP4matrix <- GROW_FP3(LIST[[i]]$FP4matrix, LIST[[i+1]]$FP4matrix, LIST[[i]]$FPALLmatrix, n=4, LIST[[i]]$TOTALP, LIST[[i]]$TOTALN) 
        LIST[[i+1]]$FP3matrix <- GROW_FP3(LIST[[i]]$FP3matrix, LIST[[i+1]]$FP3matrix, LIST[[i]]$FPALLmatrix, n=3, LIST[[i]]$TOTALP, LIST[[i]]$TOTALN) 
        LIST[[i+1]]$FP2matrix <- GROW_FP3(LIST[[i]]$FP2matrix, LIST[[i+1]]$FP2matrix, LIST[[i]]$FPALLmatrix, n=2, LIST[[i]]$TOTALP, LIST[[i]]$TOTALN) 
        LIST[[i+1]]$FP1matrix <- GROW_FP3(LIST[[i]]$FP1matrix, LIST[[i+1]]$FP1matrix, LIST[[i]]$FPALLmatrix, n=1, LIST[[i]]$TOTALP, LIST[[i]]$TOTALN)
