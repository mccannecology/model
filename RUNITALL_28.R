...
parameters <- read.csv("input_wind_size_and_shape.csv")[73:76,]
...
sourceDirectory(path=paste(getwd(),"/FUNCTIONS28",sep=""), recursive=FALSE, modifiedOnly=FALSE) 

#  assigns the functions to the global environments of each node
clusterExport(cl, c("BLANK28", "GROW_SAV28", "GROW_FP28", 
                    "INPUT28","MOVE28","OUTPUT_28",
                    "OVERWINTER28","SPECIES28","START28",
                    "STEP28","UPTAKE_N28","UPTAKE_P28","WIND28"))
...
RESULT <- foreach (i=1:nrow(parameters), .combine=rbind, .errorhandling='pass') %dopar% { # loop through all of your simulations - User needs to specify the max # of simulations (rows of parameters) in .csv
  ...
  INPUT28(simulnumb) # reads the .csv file of parameter values and assigns them to the global environment 
  
  specieslist <- SPECIES28() # function that builds the list of species-specific parameters that is used in STEPX()  
  ...
  OUTPUT_28(shapshots=TRUE,timeseries=TRUE) 
