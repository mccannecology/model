...
data <- read.csv("output_wind_and_size.csv")
...
data$size <- factor(data$size, levels=c("small","medium","large","XL"),
                    labels=c("0.04ha","1ha","6.25ha","9ha"))
...
SAV_FP_plot2 <- SAV_FP_plot2 + facet_grid(wind_direction ~ size)
...
ggsave(file="output_wind_and_size - state_score.jpg",SAV_FP_plot2, height=11,width=8)
