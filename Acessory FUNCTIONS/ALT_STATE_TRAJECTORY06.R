...
data <- read.csv("output_growing_season.csv") # imports parameter  values for all simulations 
head(data)
nrow(data)
colnames(data)
...
for (i in unique(data$TOTALN)){
  for (j in unique(data$days)) {
    
    temp_data <- subset(data, data$TOTALN == i & data$days == j)
    ...
    ggsave(file=paste("alt_states_plot",i,j,".jpg",sep="_"),temp_plot,height=6,width=6)
