...
plot_6 <- plot_6 + ggtitle(NULL) + theme(plot.title=element_text(face="bold"))
plot_7 <- plot_7 + ggtitle(NULL) + theme(plot.title=element_text(face="bold"))
plot_8 <- plot_8 + ggtitle(NULL) + theme(plot.title=element_text(face="bold"))
plot_9 <- plot_9 + ggtitle(NULL) + theme(plot.title=element_text(face="bold"))
plot_10 <- plot_10 + ggtitle(NULL) + theme(plot.title=element_text(face="bold"))

plot_1 <- plot_1 + xlab(NULL)
plot_2 <- plot_2 + xlab(NULL)
plot_3 <- plot_3 + xlab(NULL)
plot_4 <- plot_4 + xlab(NULL)
plot_5 <- plot_5 + xlab(NULL)

plot_2 <- plot_2 + ylab(NULL)
plot_3 <- plot_3 + ylab(NULL)
plot_4 <- plot_4 + ylab(NULL)
plot_5 <- plot_5 + ylab(NULL)
plot_7 <- plot_7 + ylab(NULL)
plot_8 <- plot_8 + ylab(NULL)
plot_9 <- plot_9 + ylab(NULL)
plot_10 <- plot_10 + ylab(NULL)
...
for (i in 1:nrow(data)){
  # set-up the x-values
  xvalues <- seq(0.01,0.99,0.005)
  
  # sample data 
  temp_data <- dbeta(xvalues,data[i,"a"],data[i,"b"])
  ...
  temp_plot <- ggplot(temp_data, aes(x=xvalues,y=value)) + geom_line(stat="density")
  temp_plot <- temp_plot + xlab("Wind strength")
  temp_plot <- temp_plot + ylab("Frequency")
  #temp_plot <- temp_plot + scale_x_continuous(expand=c(0,0),limits=c(0,1),breaks=c(0,0.5,1))
  #temp_plot <- temp_plot + scale_y_continuous(expand=c(0,0))
...
ggsave("wind_scaling_with_size_figure-PDF.jpg",combined_plot,height=4,width=10)
