...
parameters$avg_SAVcover_yr04 <- rep(NA, nrow(parameters))
...
parameters$avg_FPcover_yr04 <- rep(NA, nrow(parameters))
# These are only necessary if you have multiple FP spp.
parameters$final_FP01_yr04 <- rep(NA, nrow(parameters))
parameters$final_FP02_yr04 <- rep(NA, nrow(parameters))
parameters$final_FP03_yr04 <- rep(NA, nrow(parameters))
parameters$final_FP04_yr04 <- rep(NA, nrow(parameters))
...
sourceDirectory(path=paste(getwd(),"/FUNCTIONS27",sep=""),recursive=FALSE) 

#  assigns the functions to the global environments of each node
clusterExport(cl, c("BLANK27", "GROW_SAV27", "GROW_FP27", 
                    "INPUT27","MOVE27","OUTPUT27_multiple_species",
                    "OVERWINTER27","SPECIES27","START27",
                    "STEP27","UPTAKE_N27","UPTAKE_P27","WIND27"))
...
RESULT <- foreach (i=1:nrow(parameters), .combine=rbind, .errorhandling='pass') %dopar% { # loop through all of your simulations - User needs to specify the max # of simulations (rows of parameters) in .csv
  ...
  specieslist <- SPECIES27() # function that builds the list of species-specific parameters that is used in STEPX()  
  ...
  OUTPUT27_multiple_species(animate=FALSE,regimethreshold=70) 
...
parameters$avg_SAVcover_yr04 <- RESULT[,15]
parameters$avg_FPcover_yr01 <- RESULT[,16]
parameters$avg_FPcover_yr02 <- RESULT[,17]
parameters$avg_FPcover_yr03 <- RESULT[,18]
parameters$avg_FPcover_yr04 <- RESULT[,19]
# These are only necessary if you have multiple FP spp.
parameters$final_FP01_yr04 <- RESULT[,20]
parameters$final_FP02_yr04 <- RESULT[,21]
parameters$final_FP03_yr04 <- RESULT[,22]
parameters$final_FP04_yr04 <- RESULT[,23]
