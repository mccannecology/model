...
START29 <- function(x){ 
  ...
  x$TOTALN[LAND==0] <- TOTALN*1000 # then assign TOTALN to all cells that are not LAND # multiply by 1000 assumming 1 m depth 
  ...
  x$TOTALP[LAND==0] <- TOTALP*1000 # then assign TOTALP to all cells that are not LAND # multiply by 1000 assumming 1 m depth 
