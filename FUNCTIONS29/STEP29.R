...
STEP29 <- function(x,t){
  ...
  if (t %ni% winters){
    ...
    # Grow SAV #
    ############
    nextstep$SAV <- GROW_SAV29(thisstep$SAV,
                               thisstep$FPtotal,
                               thisstep$TOTALP,
                               thisstep$TOTALN,
                               LAND) 
    ...
    for (j in 1:numbFPspecies) {
      nextstep$FP[[j]]<-GROW_FP29(thisstep$FP,
                                  thisstep$FPtotal,
                                  j,
                                  thisstep$TOTALP,
                                  thisstep$TOTALN,
    ...
    if (t %in% mix_days){
      ...
      # TOTAL P # 
      # uptake 
      nextstep$TOTALP <- UPTAKE_P29(thisstep,nextstep)
      ...
      # TOTAL N # 
      # uptake 
      nextstep$TOTALN <- UPTAKE_N29(thisstep,nextstep)
    ...
    nextstep$SAV <- MOVE29(nextstep$SAV,neigh_thresh_SAV,focal_thresh_SAV,amnt_colonize_SAV)
    ...
    nextstep$FP <- lapply(nextstep$FP,function(x){
      MOVE29(x,neigh_thresh_FP,focal_thresh_FP,amnt_colonize_FP)
    })
    ...
    nextstep$FP <- lapply(nextstep$FP,function(x){
      WIND29(x,proptomove,direction_of_wind,full_thresh_wind=800) 
    })
  ...
  ##############
    ...
    for (i in 1:numbFPspecies) {
      nextstep$FP[[i]]<-OVERWINTER29(thisstep$FP,i) 
    }
