...
WIND29 <- function (x1,proptomove,direction_of_wind,full_thresh_wind=800) {
  x2 <- x1 # this will hold the values for the next time step 
  
  # new mass originating from center cells 
  new_mass_C <- matrix(0,nrow=height1,ncol=width1)
  remove_mass_C <- matrix(0,nrow=height1,ncol=width1)
  ...
  for (j in 1:height1) { # loop over all rows (height1)
    for (k in 1:width1) { # loop over all columns (width1)
      
      # find cells that are occupied 
      if (x1[j,k] > 0) {
        
        # if the cell is in the center, then do the following: 
        if ((j>=2 & j<=(height1-1)) & (k>=2 & k<=(width1-1))) {                                           
          ...
          if (direction_of_wind == "U"){  
            if (x1[j-1,k] < full_thresh_wind & LAND[j-1,k]==0) { # only move if the destination cell is not "too full" and not LAND 
          ...
          if (direction_of_wind == "R"){    
            if (x1[j,k+1] < full_thresh_wind & LAND[j,k+1]==0) { # only move if the destination cell is not "too full" and not LAND 
        ...
        else if (j==height1 & (k>=2 & k<=(width1-1))) {           
        ...
        else if (k==width1 & (j>=2 & j<=(height1-1))) { 
    ...
    } # closes for loop - through width1/columns (k)    
  } # closes for loop - through height1/rows (j)  
