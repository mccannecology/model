GROW <- function(x1,x2,x3) { # function has both LIST[[i]]$SPmatrix and LIST[[i+1]]$SPmatrix as inputs 
    ...
    for (k in 1:width) { # loop over all columns (width)
      if (x1[j,k] > 0) {
        x2[j,k] <- x1[j,k] + maxrgr*x1[j,k] * # initial biomass plus new growtht 
                      ...
                      (halfsatB/(x3[j,k]+halfsatB)) * # biomass limitation ** KEY CHANGE: replace x1[j,k] w/ a multispecies measure
