...
UPTAKE_N25 <- function(thisstep,nextstep) {
  ...
  if ((thisstep$TOTALN*(height1*width1-sum(LAND)) - totalNremoved) / 
      
        (height1*width1-sum(LAND)) >= 0){
  
    nextstep$TOTALN <- (thisstep$TOTALN*(height1*width1-sum(LAND))- totalNremoved) / 
