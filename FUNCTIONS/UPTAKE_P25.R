...
UPTAKE_P25 <- function(thisstep,nextstep) {
  ...
  if ((thisstep$TOTALP*(height1*width1-sum(LAND)) - totalPremoved) / 
        
        (height1*width1-sum(LAND)) >= 0){
    
    nextstep$TOTALP <- (thisstep$TOTALP*(height1*width1-sum(LAND))- totalPremoved) / 
      
      (height1*width1-sum(LAND))
  } 
