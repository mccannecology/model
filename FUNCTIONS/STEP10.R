STEP10 <- function() { # start defining the function 
  ...
  for (i in 1:((timesteps+1)*years)) { # loop through time steps 
    ...
    if (i %ni% winters) { 
      ...
      ########
      if (numbspecies == 4) { 
        LIST[[i+1]]$SP4matrix <- GROW2(LIST[[i]]$SP4matrix, LIST[[i+1]]$SP4matrix, LIST[[i]]$SPALLmatrix, n=4, LIST[[i]]$TOTALP, LIST[[i]]$TOTALN) 
        LIST[[i+1]]$SP3matrix <- GROW2(LIST[[i]]$SP3matrix, LIST[[i+1]]$SP3matrix, LIST[[i]]$SPALLmatrix, n=3, LIST[[i]]$TOTALP, LIST[[i]]$TOTALN) 
        LIST[[i+1]]$SP2matrix <- GROW2(LIST[[i]]$SP2matrix, LIST[[i+1]]$SP2matrix, LIST[[i]]$SPALLmatrix, n=2, LIST[[i]]$TOTALP, LIST[[i]]$TOTALN) 
        LIST[[i+1]]$SP1matrix <- GROW2(LIST[[i]]$SP1matrix, LIST[[i+1]]$SP1matrix, LIST[[i]]$SPALLmatrix, n=1, LIST[[i]]$TOTALP, LIST[[i]]$TOTALN)
