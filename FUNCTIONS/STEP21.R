...
STEP21<-function(x,t){
  ...
  if (t %ni% winters){
    ...
    # Grow SAV #
    ############
    nextstep$SAV <- GROW_SAV21(thisstep$SAV,thisstep$FPtotal,thisstep$TOTALP,thisstep$TOTALN) 
    
    ###########
    # Grow FP #
    ###########  
    for (j in 1:numbFPspecies) {
      nextstep$FP[[j]]<-GROW_FP21(thisstep$FP,thisstep$FPtotal,j,thisstep$TOTALP,thisstep$TOTALN) 
    }
    ...
    # Uptake N #
    ############
    nextstep$TOTALN <- UPTAKE_N21(thisstep,nextstep)
    
    ############
    # Uptake P #
    ############ 
    nextstep$TOTALP <- UPTAKE_P21(thisstep,nextstep)
  ...
  ##############
    ...
    for (i in 1:numbFPspecies) {
      nextstep$FP[[i]]<-OVERWINTER21(thisstep$FP,i) 
    }
