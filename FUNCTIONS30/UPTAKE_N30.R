...
UPTAKE_N30 <- function(thisstep,nextstep) {
  
  ###################################
  # Define function for uptake rate # 
  ################################### 
  uptake_rate <- function(cVNUptMax,cNDmax,cNDmin,rND,TOTALN,cAffNUpt){
    # corrected maximum uptake rate 
    uptake_max_cor <- cVNUptMax*((cNDmax-rND)/(cNDmax-cNDmin))
  ...
  NremovedSAV <- (newbiomassSAV * uptake_rate(TOTALN = thisstep$TOTALN, 
                                              cVNUptMax = cVNUptMax_SAV,
                                              cNDmax = cNDMax_SAV,
                                              cNDmin = cNDMin_SAV,
                                              cAffNUpt = cAffNUpt_SAV,
                                              rND = rNDnew_SAV) * 1000) + 
    
                  (oldbiomassSAV * uptake_rate(TOTALN = thisstep$TOTALN, 
                                               cVNUptMax = cVNUptMax_SAV,
                                               cNDmax = cNDMax_SAV,
                                               cNDmin = cNDMin_SAV,
                                               cAffNUpt = cAffNUpt_SAV,
  ...
  for (i in 1:numbFPspecies){
    ...
    NremovedFP[[i]]  <- (newbiomassFP[[i]] * uptake_rate(TOTALN = thisstep$TOTALN, 
                                                     cVNUptMax = cVNUptMax_FP,
                                                     cNDmax = cNDMax_FP,
                                                     cNDmin = cNDMin_FP,
                                                     cAffNUpt = cAffNUpt_FP,
                                                     rND = rNDnew_FP) * 1000) + 
      
                        (oldbiomassFP[[i]] * uptake_rate(TOTALN = thisstep$TOTALN, 
                                                     cVNUptMax = cVNUptMax_FP,
                                                     cNDmax = cNDMax_FP,
                                                     cNDmin = cNDMin_FP,
                                                     cAffNUpt = cAffNUpt_FP,
