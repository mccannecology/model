...
STEP30 <- function(x,t){
  ...
  if (t %ni% winters){
    ...
    # Grow SAV #
    ############
    nextstep$SAV <- GROW_SAV30(thisstep$SAV,
                               thisstep$FPtotal,
                               thisstep$TOTALN,
                               LAND) 
    ...
    for (j in 1:numbFPspecies) {
      nextstep$FP[[j]]<-GROW_FP30(thisstep$FP,
                                  thisstep$FPtotal,
                                  j,
                                  thisstep$TOTALN,
                                  LAND) 
    ...
    if (t %in% mix_days){
      ...
      # TOTAL N # 
      # uptake 
      nextstep$TOTALN <- UPTAKE_N30(thisstep,nextstep)
    ...
    nextstep$SAV <- MOVE30(nextstep$SAV,neigh_thresh_SAV,focal_thresh_SAV,amnt_colonize_SAV)
    ...
    nextstep$FP <- lapply(nextstep$FP,function(x){
      MOVE30(x,neigh_thresh_FP,focal_thresh_FP,amnt_colonize_FP)
    })
    ...
    nextstep$FP <- lapply(nextstep$FP,function(x){
      WIND30(x,proptomove,direction_of_wind,full_thresh_wind=800) 
    })
  ...
  ##############
    ...
    for (i in 1:numbFPspecies) {
      nextstep$FP[[i]]<-OVERWINTER30(thisstep$FP,i) 
    }
